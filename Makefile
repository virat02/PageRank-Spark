...
# -----------------------------------------------------------
spark.root=/home/virat/tools/spark/spark-2.3.2-bin-without-hadoop
hadoop.root=/home/virat/tools/hadoop/hadoop-2.8.5
app.name=Pagerank
jar.name=spark-hw4.jar
maven.jar.name=spark-hw4-1.0.jar
job.name=pr.Pagerank
...
aws.bucket.name=virat-hw4-spark
aws.input=input
aws.output=output
aws.log.dir=log
aws.num.nodes=5
...
local: jar clean-local-output
	spark-submit --class ${job.name} --master ${local.master} --name "${app.name}" ${jar.name} ${local.input} ${local.output} ${local.param}
...
aws: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		--name "FollowerCount Spark Cluster Run 2" \
